...
<section><h3>Claude Code</h3><p class="meta">Project <code>.mcp.json</code> shown below. For a personal installation, put the equivalent server in user scope. Keep <code>alwaysLoad</code>: it gives the remote Aura server reliable first-turn visibility even when MCP tool search is deferred. Aura also marks its reply-inbox tool with a narrower server-side loading hint where the client supports that metadata.</p><pre><code>${escapeHtml(claude)}</code></pre><p class="meta">Verify with <code>claude mcp get aura</code> or <code>/mcp</code>.</p></section>
